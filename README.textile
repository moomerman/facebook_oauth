...
You can access any object listed above in the same way via the API.  For example, event attendees would be <code>client.event('event_id').attendees
</code>.  Check out the "Facebook API Reference":http://developers.facebook.com/docs/reference/api/ to see what methods are available.